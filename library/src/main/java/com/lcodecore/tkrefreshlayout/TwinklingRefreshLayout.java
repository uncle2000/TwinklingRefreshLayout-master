...
public class TwinklingRefreshLayout extends RelativeLayout implements PullListener, NestedScrollingChild {
    ...
    protected ViewGroup mHeadLayout2;
    //是否刷新视图可见
    ...
    private List<HeaderBo> headerBoList = new ArrayList<>();
    /**
     * 设置刷新控件监听器
     */
    ...
    public TwinklingRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        headerBoList.clear();
        HeaderBo headerPull = new HeaderBo();
        HeaderBo headerSticker = new HeaderBo();

        headerPull.setHeadLayout(new FrameLayout(getContext()));
        headerPull.setUpType(HeaderBo.HEAD_PULL_TO_REF);
        headerPull.setId(R.id.header);
        headerSticker.setHeadLayout(new FrameLayout(getContext()));
        headerSticker.setUpType(HeaderBo.HEAD_STICKER);
        headerSticker.setId(R.id.header1);

        TextView tv = new TextView(getContext());
        tv.setText("xxxxxxxxx");
        headerSticker.getHeadLayout().addView(tv);

        headerBoList.add(headerPull);
        headerBoList.add(headerSticker);
    ...
    private void addHeader() {
        ...
        for (int i = 0; i < headerBoList.size(); i++) {
            LayoutParams layoutParams;
            if (i == 0) {
                layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, 0);
            } else {
                layoutParams = new LayoutParams(LayoutParams.MATCH_PARENT, DensityUtil.dp2px(getContext(), 100));
                layoutParams.addRule(ALIGN_PARENT_TOP);
                headerBoList.get(i).getHeadLayout().setBackgroundColor(0xffff0000);
            }

            this.addView(headerBoList.get(i).getMyHeader(), layoutParams);
        }

        for (HeaderBo bo : headerBoList) {
            if (bo.getUpType().equals(HeaderBo.HEAD_PULL_TO_REF)) {
                mHeadLayout = bo.getHeadLayout();
                break;
            }
        }
        for (HeaderBo bo : headerBoList) {
            if (bo.getUpType().equals(HeaderBo.HEAD_STICKER)) {
                mHeadLayout2 = bo.getHeadLayout();
                break;
            }
        }
    ...
    protected void onFinishInflate() {
        ...
        mChildView = getChildAt(headerBoList.size() + 1);//><
        LayoutParams params = (LayoutParams) mChildView.getLayoutParams();
        params.addRule(BELOW, headerBoList.get(headerBoList.size() - 1).getId());
    ...
    public void setFloatRefresh(boolean ifOpenFloatRefreshMode) {
        ...
        post(new Runnable() {
            ...
            public void run() {
                for (HeaderBo bo : headerBoList) {
                    if (bo.getHeadLayout() != null)
                        bo.getHeadLayout().bringToFront();
                }
            }
        });
    }
    ...
    public class CoContext {
        public final static int PULLING_TOP_DOWN = 0;
        public final static int PULLING_BOTTOM_UP = 1;
        public final static int PULLING_HEAD_UP = 2;
        public final static int PULLING_HEAD_DOWN = 3;
        public final static String HEAD_ALL_SHOW = "head_all_show";
        public final static String HEAD_HIDE = "head_hide";
        public final static String HEAD_SHOW_PART = "head_show_part";
        private static final int EX_MODE_NORMAL = 0;
        private static final int EX_MODE_FIXED = 1;
        public float allHeadH = 0, lastSaveH = 0;
        public int state = PULLING_TOP_DOWN;
        public String headerPos = HEAD_ALL_SHOW;
        ...
        public void init() {
            if (isPureScrollModeOn) {
                ...
                for (HeaderBo bo : headerBoList) {
                    if (bo.getHeadLayout() != null)
                        bo.getHeadLayout().setVisibility(GONE);
                }
                if (mBottomLayout != null) mBottomLayout.setVisibility(GONE);
            }
            allHeadH = DensityUtil.dp2px(getContext(), 100);
        }
        ...
        public List<HeaderBo> getHeaderList() {
            return headerBoList;
        }

        public View getHead2() {
            return mHeadLayout2;
        }
...
//            LayoutParams params = (LayoutParams) mChildView.getLayoutParams();
//            params.addRule(BELOW, mHeadLayout2.getId());
//            mChildView.setLayoutParams(params);
//            requestLayout();><
        }
        ...
        public void setStatePHU() {
            System.out.println("======================触发头部上移");
            state = PULLING_HEAD_UP;
        }

        public void setStatePHD() {
            System.out.println("======================触发头部下移");
            state = PULLING_HEAD_DOWN;
        }

        public boolean isStatePHU() {
            return PULLING_HEAD_UP == state;
        }

        public boolean isStatePHD() {
            return PULLING_HEAD_DOWN == state;
        }
