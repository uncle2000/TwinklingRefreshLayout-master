...
import com.lcodecore.tkrefreshlayout.HeaderBo;
...
import static com.lcodecore.tkrefreshlayout.TwinklingRefreshLayout.CoContext.HEAD_SHOW_PART;
...
public class AnimProcessor implements IAnimRefresh, IAnimOverScroll {
    ...
    public void scrollHeadByMove(float moveY) {
        ...
        if (scrollHeadLocked && cp.isEnableKeepIView()) {
            ...
            cp.getHeader().setTranslationY(offsetY / 3);
        ...
        if (!cp.isOpenFloatRefresh()) {
            ...
            translateHead2((int) offsetY);
        }
    }

    public void scrollStickerHeadByMoveUp(float moveY) {
        float offsetY = decelerateInterpolator.getInterpolation(moveY / -cp.allHeadH / 2) * moveY / 2;

//        System.out.println("================up=====" + offsetY);

        offsetY = Math.max(-cp.allHeadH + 1, offsetY);
        offsetY = Math.min(-1, offsetY);
        if (offsetY <= -cp.allHeadH + 1) {
            cp.headerPos = HEAD_HIDE;
            cp.lastSaveH = -cp.allHeadH*2;
        } else if (offsetY >= -1) {
            cp.headerPos = HEAD_ALL_SHOW;
            cp.lastSaveH = 0;
        } else {
            cp.headerPos = HEAD_SHOW_PART;
        }

        cp.getHead2().setTranslationY(offsetY);

//        cp.getTargetView().requestLayout();
//        cp.getTargetView().setTranslationY(offsetY);
    }

    public void scrollStickerHeadByMoveDown(float moveY) {
        float offsetY = decelerateInterpolator.getInterpolation(moveY / cp.getMaxHeadHeight() / 2) * moveY / 2;
//        System.out.println("================down=====" + offsetY);

        offsetY = Math.min(cp.allHeadH - 5, offsetY);
        offsetY = Math.max(0 + 5, offsetY);
        if (offsetY >= cp.allHeadH - 5) {
            cp.headerPos = HEAD_HIDE;
        } else if (offsetY <= 5) {
            cp.headerPos = HEAD_ALL_SHOW;
        } else {
    ...
    private void translateExHead(int offsetY) {
        if (!cp.isExHeadLocked()) {
            for (HeaderBo bo : cp.getHeaderList()) {
                if (bo.getUpType().equals(HeaderBo.HEAD_STICKER)) {
                    bo.getHeadLayout().setTranslationY(offsetY);
                }
            }
        }
    }

    private void translateHead2(int offsetY) {
        cp.getHead2().setTranslationY(offsetY);
    }
